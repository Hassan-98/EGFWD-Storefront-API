...
describe('Test User Routes' , () => {
  ...
  it('test get all user route "GET: /api/users"', async () => {
    const response = await request.get('/api/users').set('Authorization', `Bearer ${TestingToken}`);
  ...
  it('test get a user route "GET: /api/users/1"', async () => {
    const response = await request.get('/api/users/1').set('Authorization', `Bearer ${TestingToken}`);
  ...
  it('test delete a user route "DELETE: /api/users/:id"', async () => {
    const response = await request.delete('/api/users/1').set('Authorization', `Bearer ${TestingToken}`);
